...
interface SceneDetailSidebarProps {
  ...
  group?: SceneGroup
  onClose: () => void
  onBanToggle?: (isBanned: boolean) => void
  isBanned?: boolean
...
export const SceneDetailSidebar: FC<SceneDetailSidebarProps> = ({
  ...
  const parcelGroupInfo = useMemo(() => {
    ...
    const sceneParcels = sceneInfo.parcels
    ...
    for (const g of existingGroups) {
      ...
      const overlap = sceneParcels.some(p => groupParcelSet.has(`${p.x},${p.y}`))
      if (overlap) {
        belongsToGroup = g
        break
      }
    ...
    const freeParcels = sceneParcels.filter(p => !allGroupParcels.has(`${p.x},${p.y}`))
  ...
  const sidebarContent = (
    <div className={styles.sidebar}>
      ...
      <div className={styles.footer}>
        ...
        >
          {isBanned ? 'Unban Scene' : 'Ban Scene'}
        </button>
      </div>
    </div>
  )
